...
# This doesn't work
#install.packages("propr/propr_4.2.6.tar.gz", repos = NULL, type = "source")
...
# Get library as a column
TPM_metadata <- TPM_metadata %>%
  mutate(Identifier = str_extract(Name, "^[^sTS]+"))

# Glimpse the data to verify
glimpse(TPM_metadata)


rm(dir_path, stage_order, file_path)
```
...
Remove the replicates that they don't use in the paper. I need to read in Supplementary Table2. Also remove samples with high ribo content (>10%)
...
# Define the file path
file_path <- file.path(dir_path, "SuppTable2.csv")

# Read the CSV file, select specific columns, and rename them
SuppTable2 <- read_csv(file_path, show_col_types = FALSE)

used_library <- SuppTable2 %>%
  filter(`Used library?` == "Yes") %>%
  pull("library ID") %>%
  as.list()


TPM_metadata <- TPM_metadata %>%
  filter(Identifier %in% used_library)


libraries_with_high_ribo <- c("5571", "5569", "6100")
TPM_metadata <- TPM_metadata %>%
  filter(!(Identifier %in% libraries_with_high_ribo))


rm(SuppTable2, used_library, libraries_with_high_ribo, dir_path, file_path)
```
...
rm(summary_metadata_organ, summary_metadata_stage)
```

## Read in gene counts

Gene counts were calculated with salmon using the nextflow pipeline. Results were written here and aggregated to one file with all samples: /igm/projects/Audrey_lncRNA/results-ribo/salmon/
Aggregated results have been copied to the InputData folder.
...
# Read in gene count data and fix column header
gene_counts <- read_tsv(
  file.path(dir_path, "salmon.merged.gene_counts.tsv"),
  show_col_types = FALSE
)

names(gene_counts)[1] <- "ENSG_ID"

# Reorder the columns in gene_counts to match the order of samples
# Select gene_id first, then reorder the remaining columns based on sample_order
gene_counts <- gene_counts %>%
  dplyr::select(ENSG_ID, all_of(TPM_metadata$Sample))

# Create a gene type column, indicating whether the gene is a lncRNA or a protein coding gene
gtf <- import('/igm/projects/Audrey_lncRNA/data/nextflow_input/gencode.v47.annotation.gtf.gz')
genes <- as.data.frame(gtf[gtf$type == "gene", c("gene_id", "gene_type")])
gene_counts$gene_type <- genes$gene_type[match(gene_counts$ENSG_ID, genes$gene_id)]
gene_counts <- gene_counts %>%
  dplyr::select(ENSG_ID, gene_type, everything())


# Summarize gene types
summary_tpm_gene <- gene_counts %>%
  group_by(gene_type) %>%
  summarise(Count = n(), .groups = 'drop') %>%
  arrange(desc(Count)) %>%
  # Use tibble::add_row() to append a total row with the sum of counts
  add_row(gene_type = "Total", Count = sum(.$Count))
style_kable(summary_tpm_gene, caption = "Genes by type")

# Display the first few rows of the CCVM data as a nicely formatted table
style_kable(gene_counts[1:5, c(1, 3, 4, 5, 6, 7)], caption = "Summary of the first few rows of the RNA-seq data")

rm(gtf, summary_tpm_gene)
```


Just keep protein coding and lncRNA genes

```{r}
gene_counts <- subset(gene_counts, gene_type %in% c("protein_coding", "lncRNA"))
```

At this point, if doing proportions rather than correlations, switch to calculating proportions in the R script. This requires more memory and time so it cannot be done in the notebook.

Here are some code scraps I use to format the data properly before submitting

```{r}
counts <- gene_counts[, -c(1, 2)]
counts <- as.data.frame(t(counts))
colnames(counts) <- gene_counts$ENSG_ID

# Dynamic filtering
keep <- colSums(counts >= 10) >= 10  # Features with at least 10 counts in at least 10 samples
filtered_indices <- which(keep)
filtered_indices <- unname(filtered_indices)
...
if (length(select) == 1 && is.na(select)) {
  ...
  counts_new <- counts[, select, drop = FALSE]
} else {
...
Proceed to next step for calculating correlations.

At this point we are following the steps in the paper describing the best way to process input data for network analysis
    1. If gene counts are available, use CTF or CUF to normalize the data. They consistently give the best performance regardless of various factors. Between the two, CTF seems to be slightly more consistent in yielding top performance. Even though no normalization (Counts) leads to good performance in our study, applying the additional normalization step is prudent to ensure robustness against variabilities specific to a new dataset.

    2. If data is only available after within-sample normalization, use TPM for coexpression analysis. Data in CPM and RPKM units can be easily converted to TPM. TPM outperforms CPM and RPKM and yields consistently reasonable performance.

    3. After normalization, perform log transformation (using asinh) and calculate coexpression using Pearson correlation coefficient.

    4. If the dataset has greater than 40 samples, use CLR to transform the pairwise gene correlations. CLR may also help certain cases where the main interest is interactions that are specific to a given tissue.



0. exclude genes that did not reach a minimum of 10 reads in at least 3 libraries

```{r}
gene_counts_input <- gene_counts[, -2]
gene_counts_input <- gene_counts_input %>%
  column_to_rownames(var = names(gene_counts_input)[1]) %>%  # Use the first column as row names
  dplyr::select(-1)   
keep_genes <- rownames(gene_counts_input)[rowSums(gene_counts_input >= 10) >= 3]
gene_counts_input <- gene_counts_input[rownames(gene_counts_input) %in% keep_genes, ]
...
dge <- DGEList(counts = gene_counts_input)
...
# Compute TMM-normalized counts
ctf <- cpm(dge, normalized.lib.sizes = TRUE)
ctf <- as.data.frame(ctf)

rm(dge)
```

2. log transform and calculate correlations

```{r}
# Perform asinh transformation
log_ctf <- asinh(ctf)

# Calculate Pearson correlation matrix (genes vs genes)
#cor_matrix <- cor(t(log_ctf), method = "pearson")

```


Option to do deseq2 vst transformation. This was for input to the PCA. Don't do this for the WGCNA.

```{r}
sample_info <- TPM_metadata %>%
  dplyr::filter(Sample %in% colnames(gene_counts_input)) %>%
  dplyr::select(Sample, Stage, Age, Organ, Identifier) %>%
  column_to_rownames(var = "Sample") 

counts_matrix <- as.matrix(gene_counts_input)
counts_matrix <- round(counts_matrix)

# Check for non-integers to ensure all are properly rounded
any(counts_matrix %% 1 != 0)  # Should return FALSE


#dir_path <- "/igm/projects/Audrey_lncRNA/results-ribo/salmon"
#files <- file.path(dir_path, rownames(sample_info), "quant.genes.sf")
#names(files) <- rownames(sample_info)  # Use sample names as names for the files
...
# Doesn't use condition so it doesn't matter what it is
...
vst_df <- vst_df %>%
  left_join(TPM_metadata %>% 
    dplyr::select(Sample, Age, Organ, Identifier), by = "Sample") %>%
  relocate(Sample, Age, Organ, Identifier, .before = everything())
...
pca_plot_data <- pca_data %>%
  left_join(TPM_metadata, by = "Sample")

pca_plot_data$Stage_numeric <- as.numeric(pca_plot_data$Stage)

min_val <- 0.5  # Desired minimum value
max_val <- 2.5 # Desired maximum value

pca_plot_data$Stage_scaled <- ((pca_plot_data$Stage_numeric - min(pca_plot_data$Stage_numeric, na.rm = TRUE)) /
                                (max(pca_plot_data$Stage_numeric, na.rm = TRUE) - min(pca_plot_data$Stage_numeric, na.rm = TRUE))) *
                                (max_val - min_val) + min_val

#custom_palette <- adjustcolor(c(
#  "cerebellum" = "#33cbff",
#  "brain" = "#3197ca",
#  "heart" = "#cb0200",
#  "liver" = "#329800",
...
# Create the PCA plot
ggplot(pca_plot_data, aes(x = PC1, y = PC2, color = Organ, size=Stage_scaled)) +
...
# Create the PCA plot
ggplot(pca_plot_data, aes(x = PC1, y = PC3, color = Organ, size=Stage_scaled)) +
  geom_point(alpha = 0.8) +
  scale_size_identity() +
  scale_color_manual(values = custom_palette,
                     guide = guide_legend(override.aes = list(alpha = 0.8))) +
...
scree_data <- data.frame(
  PC = seq_along(explained_variance),         # Principal component index
  Variance = explained_variance * 100         # Variance as percentage
)
...
heart_samples <- intersect(heart_samples, colnames(log_ctf))


# Create a list of values filtered for lncRNA expression in the heart
# We can pick this cutoff based on the pre-log transformed ctf and then apply it to the post transformed.
subset_df <- ctf[, heart_samples]
no_high_expression <- rowSums(subset_df > 1) > 0
ctf_filtered_heart <- log_ctf[no_high_expression, ]

rm(ctf, subset_df, log_ctf)
```
...
```{r}
#ctf_input <- as.data.frame(t(ctf_filtered_heart))
ctf_input <- as.data.frame(t(log_ctf))
gsg <- goodSamplesGenes(ctf_input)
print(summary(gsg))
```
...
sft <- pickSoftThreshold(ctf_input, 
                         powerVector = powers, 
                         verbose = 5,
...
CairoPDF(file = "Figures/Audrey_Analysis/Figure_S5_WGCNA_Power_Analysis.pdf",
...
I did this step in an R script because it took a long time (CCVM_Script_scorrelations_1B.R). The output was saved to dissTOM.rds
...
TOM = TOMsimilarityFromExpr(ctf_input,
                            power = 9,
...
rownames(TOM) <- colnames(ctf_input)
colnames(TOM) <- colnames(ctf_input)
...
```{r}
dissTOM <- readRDS("dissTOM_39567genes.rds")
...
```{r}
# Base R heatmap
png("Figures/Audrey_Analysis/Figure_S7_Clustering_of_Module_Eigengenes.png", width = 800, height = 600)
heatmap(
  TOM,
...
CairoPDF(file = "Figures/Audrey_Analysis/Figure_S6_WGCNA_Gene_Clustering_TOM_Dissimilarity_39567genes.pdf",
...
MEList <- moduleEigengenes(ctf_input, colors = dynamicColors)
...
mergedModules <- mergeCloseModules(ctf_input, 
                                   dynamicColors, 
...
gene_module_assignment_merged <- tibble(
  gene_id = colnames(ctf_input),        # Gene identifiers from TPM
...
ggsave("Figures/Audrey_Analysis/Figure_S7_Clustering_of_Module_Eigengenes_39567genes.pdf", 
...
#ctf_gene_type <- gene_counts %>% dplyr::select(ENSG_ID, gene_type) %>%
#  filter(ENSG_ID %in% rownames(ctf_filtered_heart)) %>%
#  rename(ensg_id = ENSG_ID)

ctf_gene_type <- gene_counts %>% dplyr::select(ENSG_ID, gene_type) %>%
  filter(ENSG_ID %in% colnames(ctf_input)) %>%
  rename(ensg_id = ENSG_ID)

#ctf_genes <- genes[genes$gene_id %in% rownames(ctf_filtered_heart), c("gene_id", "gene_type")]
ctf_genes <- genes[genes$gene_id %in% colnames(ctf_input), c("gene_id", "gene_type")]


# Get back the Gene name from the input data. We don't need the counts but just the identifiers
dir_path <- "InputData"

gene_ensg_map <- read_tsv(
  file.path(dir_path, "salmon.merged.gene_counts.tsv"),
  show_col_types = FALSE,
  col_select = 1:2 
)
...
gene_info <- ctf_genes %>% 
  left_join(gene_ensg_map, by = "gene_id") %>%
...
# 140 genes overlap
length(intersect(gene_info$gene_name, chd_gene_list))
# "NODAL" "ZIC3" are not in our results genes
setdiff(chd_gene_list, gene_info$gene_name)


# Add column for CHD gene to gene_info table
gene_info <- gene_info %>% 
  mutate(chd_gene = gene_name %in% chd_gene_list) %>%
...
geneCounts <- gene_info %>%
  ...
  summarise(
    ...
    protein_count = sum(gene_type == "protein_coding"),
...
write.csv(moduleTraitCor, "moduleTraitCor_39567genes.csv")
```
...
CairoPDF(file = "Figures/Audrey_Analysis/Figure_3_Module_Organ_Relationships_39567genes.pdf", 
         width = 8.5, height = 11)
...
CairoPDF(file = "Figures/Audrey_Analysis/Figure_S8_Module_Organ_Eigengene_Dendrogram_39567genes.pdf", 
         width = 11, height = 8.5)
...
CairoPDF(file = "Figures/Audrey_Analysis/Figure_S9_Module_Organ_Eigengene_Adjacency_heatmap_39567genes.pdf", 
...
# Save gene_info
write.csv(
  gene_info,                         # Your data frame
  file = "gene_info_module_results_39567genes.csv",
  row.names = FALSE
)
...
# Step 3: Open a Cairo PDF device with a defined page size
CairoPDF(file = "Figures/Audrey_Analysis/Figure_S10_Module_Heart_Relationships_39567genes.pdf", 
         width = 5, height = 11.5)
par(mar = c(5, 10, 5, 5))  # mar = c(bottom, left, top, right) 
...
# Plot the eigengene dendrogram
CairoPDF(file = "Figures/Audrey_Analysis/Figure_S11_Module_Heart_Eigengene_Dendrogram_39567genes.pdf", 
         width = 11, height = 8.5)
par(mar = c(5, 5, 5, 5))  # mar = c(bottom, left, top, right) 
par(cex = 1.0)  # Set the character expansion for the plot
...
# Plot the eigengene adjacency heatmap
CairoPDF(file = "Figures/Audrey_Analysis/Figure_S12_Module_Heart_Eigengene_Adjacency_heatmap_39567genes.pdf", 
         width = 8.5, height = 8.5)
par(mar = c(5, 10, 5, 5))  # mar = c(bottom, left, top, right) 
par(cex = 1.0)
...
heart_Age <- stageData[match(heart_samples, rownames(stageData)), , drop = FALSE]
```
...
if (!identical(rownames(Heart.dat), rownames(ctf_input))) {
  stop("The row names (sample identifiers) of MEs_ordered and traitData do not match or are not in the same order.")
}

# Calculate gene module membership and p-values
geneHeartCor <- WGCNA::cor(ctf_input, Heart.dat, use = "p")
...
```{r}
# Check if the row names of mergedMEs and TPM_transposed match and are in the same order
if (!identical(rownames(mergedMEs), rownames(ctf_input))) {
  stop("The row names (sample identifiers) of MEs_ordered and traitData do not match or are not in the same order.")
}

# Calculate gene module membership and p-values
geneModuleMembershipCor <- WGCNA::cor(ctf_input, mergedMEs, use = "p")
...
```{r}
write.csv(
  gene_info_combined,                         # Your data frame
  file = "gene_info_module_results_cutoff_39567genes.csv",
  row.names = FALSE
)
```
...
```{r}
plot_eigengenes <- function(MEs_ordered, ctf_input,
  ...
  # Save the plot
  ggsave(paste0("Figures/Audrey_Analysis", figureName, "_", module, 
